...
// Initial permutation table for data
int IP[] = {
...
// Final permutation table for data (inverse of IP)
int FP[] = {
...
// S-boxes for substitution in Feistel function
int S[8][4][16] = {
    {   // S1
...
// Permutation table for Feistel function
int P[32] = {
...
// Permutes a key or data block using the given permutation table
...
// Performs a circular left shift on the given key
...
// Encrypts a block of data using DES
uint64_t desEncryptBlock(uint64_t input, uint64_t* subkeys) {
    ...
    // No swap after final round
    uint64_t combined = (right << 32) | left;  // Swap left and right here
    return permute(combined, FP, 64);
}

// Decrypts a block of data using DES
...
// 3DES encryption: Encrypt-Decrypt-Encrypt with three keys
...
// 3DES decryption: Decrypt-Encrypt-Decrypt with three keys
